<?php namespace Distilleries\Expendable\Helpers;
      ...
      use Distilleries\Expendable\Exceptions\Jsv4Error;
      ...
      class Jsv4 {
          ...
          static public function isValidDatabaseMember($data, $schema, $member, $webData) {

              if ( !isset($data->{$member}) ) {
                  if ( $schema->schema->properties->{$member}->required ){
                      return false;
                  }

                  if ( isset($webData->{$member}) ) {
                      return false;
                  }
                  ...
                  $result = new Jsv4( $data->{$member},  $schema->schema->properties->{$member}, true);

                  if($result->valid){
                      return $data->{$member} === $webData->{$member};
                  }else{
                      return false;
                  }
              }
          ...
          static public function isValidMember($data, $schema, $member) {

              if ( !isset($data->{$member}) ) {
                  if ( isset($schema->schema->properties->{$member}->required ))
                  {
                      if ( $schema->schema->properties->{$member}->required ){
                          return false;
                      }
          ...
          private function checkString() {
              ...
              if (isset($this->schema->format)) {
                  ...
                  //IMPORTANT! it works only with date format MMDDYYYY
                  if ($this->schema->format == 'date'){
                      $d = \DateTime::createFromFormat('mdY', $this->data);
                      if (!($d && $d->format('mdY') == $this->data)) {
                          $error = true;
                      }

                  }elseif($this->schema->format == 'email'){
                  ...
                  if ($error) {
                      $this->fail(self::JSV4_STRING_FORMAT, "", "/format", "String does not match format {$this->schema->format}");
                  }
              }
          }
