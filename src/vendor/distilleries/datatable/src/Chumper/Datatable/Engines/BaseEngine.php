      ...
      abstract class BaseEngine {
          ...
          public function showColumns($cols)
          {
              ...
              foreach ($cols as $property) 
              {
                  ...
                  {
                      $this->columns->put($property, 
                          new FunctionColumn($property, 
                              function($model) use($property)
                              {
                                  try
                                  {
              ...
              foreach($this->columns as $name => $column)
              {
                  if($i == $value && in_array($name, $cleanNames))
                  {
                      $this->order(
                          [
                            0 => $value, 
                            1 => $this->orderColumns[array_search($name, $cleanNames)]
                          ], $direction);
                      return;
                  }
                  $i++;
