      ...
      class QueryEngine extends BaseEngine {
          ...
          private function buildSearchQuery($builder, $columns)
          {
              ...
              $builder = $builder->where(
                  function($query) use ($columns, $search, $like, $exact) 
                  {
                      foreach ($columns as $c) 
                      {
                          ...
                          elseif(in_array($c, array('created_at', 'updated_at')))
                          {
                              //TODO: Implement this
                              //$this->columns->put($property, new DateColumn($property, DateColumn::DAY_DATE));
                              //$query->orWhereRaw('STR_TO_DATE('.$c.',%Y-%m-%d %H:%i:%s) '.$like.' ?', array($exact ? "$search" : "%$search%"));
