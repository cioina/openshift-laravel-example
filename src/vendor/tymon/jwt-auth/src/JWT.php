...
class JWT
{
    ...
    public function fromSubject(JWTSubject $subject)
    {
        if(!$this->token)
        {
            $payload = $this->makePayload($subject)->get();
        }else{
            $oldPayload = $this->payload()->get();
            $payload = $this->makePayload($subject)->get();

            if(!Utils::timestamp($oldPayload['exp'])->subMinutes((int)config('jwt.ttl')/2)->isPast())
            {
                $payload['exp'] = $oldPayload['exp'];
                $payload['iat'] = $oldPayload['iat'];
            }
        }
