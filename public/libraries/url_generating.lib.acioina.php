 ...
 * @access  public
 */
function CIOINA_URL_getHiddenInputs($db = '', $table = '',
    $indent = 0, $skip = array()
) {
    if (is_array($db)) {
        $params  =& $db;
    ...
    if (empty($_COOKIE['CIOINA_lang']) && ! empty($GLOBALS['lang'])) {
        $params['lang'] = $GLOBALS['lang'];
    }
    if (empty($_COOKIE['CIOINA_collation_connection'])
        && ! empty($GLOBALS['collation_connection'])
    ) {
        $params['collation_connection'] = $GLOBALS['collation_connection'];
    }

    $params['token'] = $_SESSION[' CIOINA_token '];
    ...
    return CIOINA_getHiddenFields($params);
}
 ...
 *     ),
 * );
 * echo CIOINA_getHiddenFields($values);
 *
 * // produces:
 ...
 */
function CIOINA_getHiddenFields($values, $pre = '')
{
    $fields = '';
    ...
    foreach ($values as $name => $value) {
        ...
        if (is_array($value)) {
            $fields .= CIOINA_getHiddenFields($value, $name);
        } else {
            // do not generate an ending "\n" because
            // CIOINA_URL_getHiddenInputs() is sometimes called
 ...
 * $params['table']   = 'rights';
 * // note the missing ?
 * echo 'script.acioina.php' . CIOINA_URL_getCommon($params);
 * // produces with cookies enabled:
 * // script.acioina.php?myparam=myvalue&amp;db=mysql&amp;table=rights
 * // with cookies disabled:
 * // script.acioina.php?server=1&amp;lang=en&amp;myparam=myvalue&amp;db=mysql
 * // &amp;table=rights
 *
 * // note the missing ?
 * echo 'script.acioina.php' . CIOINA_URL_getCommon();
 * // produces with cookies enabled:
 * // script.acioina.php
 * // with cookies disabled:
 * // script.acioina.php?server=1&amp;lang=en
 * </code>
 *
 ...
 * @access  public
 */
function CIOINA_URL_getCommon($params = array(), $encode = 'html', $divider = '?')
{
    ...
    if (isset($_SESSION[' CIOINA_token '])) {
        $params['token'] = $_SESSION[' CIOINA_token '];
    }
 ...
 * @access  public
 */
function CIOINA_URL_getArgSeparator($encode = 'none')
{
