...
class CIOINA_OutputBuffering
{
    ...
    private function _getMode()
    {
        ...
        if ($GLOBALS['cfg']['OBGzip'] && function_exists('ob_start')) {
            if (ini_get('output_handler') == 'ob_gzhandler') {
                ...
                // any right frame file in ACIOINA will not be handled properly by
                // the browser. My fix was to check the ini file within the
                // CIOINA_outBufferModeGet() function.
                $mode = 0;
    ...
    /**
     * Returns the singleton CIOINA_OutputBuffering object
     *
     * @return CIOINA_OutputBuffering object
     */
    public static function getInstance()
    {
        if (empty(self::$_instance)) {
            self::$_instance = new CIOINA_OutputBuffering();
        }
     ...
     * the CIOINA_outBufferModeGet() function or it will be useless.
     *
     * @return void
     */
    public function start()
    {
        if (! $this->_on) {
            ...
            register_shutdown_function(
                array('CIOINA_OutputBuffering', 'stop')
            );
            $this->_on = true;
        }
    }
     ...
     * CIOINA_outBufferModeGet() function or it will be useless.
     *
     * @return void
     */
